...
namespace BarRaider.SdTools.Communication
{
    ...
    public class StreamDeckConnection
    {
        ...
        internal Task SendAsync(IMessage message)
        {
            try
            {
                return SendAsync(JsonSerializer.Serialize(message, SourceGenerationContext.Default.IMessage));
            }
            catch (Exception ex)
            {
        ...
        internal Task SetGlobalSettingsAsync(JsonObject settings)
        {
        ...
        internal Task SetSettingsAsync(JsonObject settings, string context)
        {
        ...
        internal Task SendToPropertyInspectorAsync(string action, JsonObject data, string context)
        {
        ...
        internal Task SetFeedbackAsync(JsonObject feedbackPayload, string context)
        {
        ...
        private async Task<WebSocketCloseStatus> ReceiveAsync()
        {
            ...
            StringBuilder textBuffer = new StringBuilder(BUFFER_SIZE);
                            ...
                            if (result.EndOfMessage)
                            {
                                ...
                                try
                                {
                                    switch (evt.Event)
                                    {
                                        ...
                                        case EventTypes.DIAL_PRESS: /* Ignoring deprecated Stream Deck event;*/ break;
                                        case EventTypes.TOUCHPAD_PRESS: OnTouchpadPress?.Invoke(this, new SdEventReceivedEventArgs<TouchpadPressEvent>(evt as TouchpadPressEvent)); break;
                                        default:
