...
namespace BarRaider.SdTools.Backend
{
    ...
    public class SdConnection : ISdConnection
    {
        ...
        /// </summary>
        [JsonIgnore]
        private readonly StreamDeckInfo.StreamDeckInfo deviceInfo;
        ...
        public SdConnection(StreamDeckConnection connection, string pluginUuid, StreamDeckInfo.StreamDeckInfo deviceInfo, string actionId, string contextId, string deviceId)
        {
        ...
        /// <returns></returns>
        public async Task SendToPropertyInspectorAsync(JsonObject settings)
        {
        ...
        /// <param name="settings"></param>
        /// <returns></returns>
        public async Task SetSettingsAsync(JsonObject settings)
        {
            if (StreamDeckConnection != null && !string.IsNullOrEmpty(ContextId) && !string.IsNullOrEmpty(actionId))
            {
        ...
        /// <returns></returns>
        public async Task SetGlobalSettingsAsync(JsonObject settings, bool triggerDidReceiveGlobalSettings = true)
        {
        ...
        {
            string hash = Tools.Tools.StringToSha512(base64Image);
        ...
        {
            string hash = Tools.Tools.ImageToSha512(image);
            if (forceSendToStreamdeck || hash != previousImageHash)
            {
                previousImageHash = hash;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task SetFeedbackAsync(JsonObject feedbackPayload)
        {
        ...
        private void Connection_OnSendToPlugin(object sender, SdEventReceivedEventArgs<SendToPluginEvent> e)
        {
            if (e.Event.Context == ContextId)
            {
                OnSendToPlugin?.Invoke(this,
                    new SdEventReceivedEventArgs<SendToPlugin>(
                        new SendToPlugin(
                            e.Event.Action,
                            e.Event.Context,
                            e.Event.Payload
